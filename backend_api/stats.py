...
async def _ensure_vk_hierarchy_rows_for_campaign(
    ...
    *,
    ...
    from backend_api.vk_hierarchy_read import ensure_vk
    if not campaign or not campaign.integration:
        return
    if campaign.integration.platform != models.IntegrationPlatform.VK_ADS or not campaign.external_id:
        return
    campaign_id, client_id = campaign.id, campaign.integration.client_id
    await ensure_vk(db, campaign_id, client_id, d_start, d_end,
                    include_ads=include_ads, user_id=user_id)
