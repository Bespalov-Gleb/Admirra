    ...
    *,
    ...
    try:
        ...
        for counter_id in all_counter_ids:
            ...
            goals_data = await queue.enqueue(
                'metrica', metrika_api.get_goals_stats, counter_id, sync_date_from, sync_date_to,
                metrics=metrics, direct_traffic_only=direct_traffic_only,
            )
            ...
            if not goals_data and goals_for_aggregate:
                ...
                goals_data = await queue.enqueue(
                    'metrica', metrika_api.get_goals_stats, counter_id, sync_date_from, sync_date_to,
                    metrics="ym:s:sumGoalVisitsAny", direct_traffic_only=direct_traffic_only,
                )
                ...
                for idx, goal_id in enumerate(goals_to_sync_individual):
                    try:
                        ...
                        goal_data = await queue.enqueue(
                            'metrica', metrika_api.get_goals_stats, counter_id, sync_date_from, sync_date_to,
                            metrics=goal_metrics, direct_traffic_only=direct_traffic_only,
                        )
    ...
    try:
        if integration.platform == models.IntegrationPlatform.YANDEX_DIRECT:
            ...
            if not integration.access_token:
                logger.warning(f"No access token for Metrica integration {integration.id}")
                integration.error_message = "No access token configured"
            ...
            access_token = security.decrypt_token(integration.access_token)
            selected_profile = metrika_profile_login(integration)
            logger.info(
                f"Syncing Yandex Metrika integration {integration.id} with profile: {selected_profile}"
            )
