...
8. На том же восстановленном наборе одновременно запущен полный стартовый consumer set: manual concurrency 2, nightly/backfill concurrency 2, reports concurrency 1 и maintenance concurrency 1. Все процессы прошли preflight и `ready`, каждый отдельный hostname ответил на Celery inspect ping через одноразовый Redis в том же `network=none`; scheduler и business jobs не запускались. Вместе с application smoke — pass, 41 s, после cleanup осталось 0 временных containers/volumes/runtime directories.
9. При одновременно работающем consumer set кандидат API с pool `5/0` обслужил 40 авторизованных read-запросов concurrency 4 к auth/clients/folders/notifications/dashboard summary: 40 × 200, p50 129,36 ms, p95 692,12 ms, max 807,58 ms, 19,12 req/s. Полный цикл — 44 s; details и ограничения: [read-load evidence](devops-read-load-2026-09-20.md).
