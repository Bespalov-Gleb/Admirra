...
- VK сохраняет objective/состав лидов/lead-form autodefault и уведомления. Ненулевые конверсии без подтверждённого objective не коммитятся. Явное `vk.goals=0` не заменяется `base.goals`. CPC/CPA провайдера сохраняются; расчёт используется только как fallback, как в прежнем обработчике.
