...
- В финальной внешней проверке маскота один запрос с локального компьютера
  завершился connect timeout (HTTP 000, не ответ приложения). Повтор с ограничением
  времени дал HTTP 200 за 0.47 s; с API1 — 200 за 0.10 s. Причина единичного
  сетевого сбоя не установлена; это не доказательство доступности со всех сетей.
  На 08:44:17 UTC активных алертов нет.
