...
### Решение владельца 23.09: модуль проверки заявок отложен

Телефония/Lead Validator (формы сайтов, антиспам, quality/blacklist, CRM/email/TG
экспорты заявок) исключён из обязательной функциональности ближайшего релиза.
Не продолжать его ранние `_reject`, обогащение, exports и diagnostics ради выхода
основного продукта в production. Сделанные commits сохраняются, задачи остаются
отложенными, а не выполненными.
...
нужен только защитный выключатель: закрыть входы отложенного модуля, не планировать
его jobs, проверить queued/running/uncertain `lead.*` и безопасно drain/изолировать
их без удаления evidence и слепых повторов. Существующие проекты/данные не удалять.
...
Этим решением production/env/обработчики **ещё не изменены**; отключение надо
реализовать и проверить в candidate перед cutover. Прежние пункты ниже про
«оставшиеся lead exports/validation» теперь относятся к отложенному модулю.
В активном плане: оставшиеся AI/report/billing внешние вызовы, операторская приёмка,
