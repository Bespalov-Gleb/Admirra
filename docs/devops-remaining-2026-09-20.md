...
**24.09, 14:10 МСК — production cutover завершён:** новая схема `f68b92a3b4c5`, API1/API2,
...
владельцу прошли. Ступени публичной балансировки 10→25→50% завершены,
веса 1:1; финальная выдержка 30 min: 720/720 controlled HTTP200, публичных
HTTP5xx=0, фактическая доля API2 49.22%. S3/PITR и две ночи остаются отложенными.
Отдельно зафиксированы три failed historical backfill тестового аккаунта:
повторное чтение прошло, исходные задания не перезапускались; это follow-up
по полноте истории, а не незавершённый cutover. Автоматического failover основной
БД нет, shared read cache выключен по launch profile.
