...
Владелец впоследствии подтвердил, что регистрация Яндекс работает. Изменений
OAuth по результатам этой проверки не делалось.

Дополнение `1deb08b`: sticky-шапка документов теперь без верхнего зазора,
с непрозрачным фоном, чтобы текст не проходил над навигацией. Версия CSS URL
`sticky-20260924`. Новый frontend image:
`sha256:f6ffb5227ecc9475cd814ae90d4cb5d006aac9e9f5845c309b8314d9d904c809`;
active/previous configs в `/etc/admirra/releases/legal-sticky-1deb08b/`.
5 Node tests passed; browser scroll checks на 1440/390 px для трёх документов:
начало, середина и конец страницы, header и inner top=0, overflow отсутствует,
мобильное меню доступно после прокрутки. Прежняя branding-приёмка ниже сохранена.
