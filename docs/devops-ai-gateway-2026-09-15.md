...
> Обновление 21.09.2026: API-2 уже подключён отдельным peer `10.78.0.2/32`; исторические пункты ниже о неподключённом втором узле больше не актуальны. [Проверки/откат](devops-progress-2026-09-21.md). Telegram alerts и внешний heartbeat также подключены: [evidence](devops-telegram-alerts-2026-09-20.md).
...
- Сохранены узкие правила DHCP-client для `eth0` (67→68, link-local IPv6 547→546), поскольку сеть провайдера получает адрес/маршрут через DHCP. Продление аренды не должно блокироваться firewall.
...
- На финальной проверке apt предлагает обновления четырёх dracut-пакетов (security/updates) и четырёх netplan-пакетов. После включения рабочего маршрута они не устанавливались: обновление boot/network стека и повторный reboot выполнять в отдельное окно с откатом маршрута/контролем консоли. Nginx и WireGuard установлены из актуального apt index этого этапа.
