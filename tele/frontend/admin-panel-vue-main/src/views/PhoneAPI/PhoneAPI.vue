...
// API base URL (настройте под ваш сервер)
...
const webhookUrls = ref({
  tilda: `${API_BASE}/webhook/tilda/`,
  marquiz: `${API_BASE}/webhook/marquiz/`
})
...
async function validateLead() {
  ...
  try {
    ...
    if (testMode.value) {
      ...
      const params = new URLSearchParams({ phone: form.value.phone })
      if (form.value.email) params.append('email', form.value.email)
      if (form.value.name) params.append('name', form.value.name)
      
      endpoint = `${API_BASE}/api/lead/test-validate?${params.toString()}`
      response = await fetch(endpoint, { method: 'POST' })
    } else {
      endpoint = `${API_BASE}/api/lead/`
